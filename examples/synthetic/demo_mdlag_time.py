"""Original mDLAG (time-domain, dense GP) on a 3-region scenario.

Scenario: 3-region synthetic data with ``K_true = 2`` ground-truth
across-latents and a constant inter-region delay. The model is built
with ``K_init = 4``; the ARD prior on the loading matrix should
soft-prune the two spurious columns, exercising mDLAG's automatic
latent-count selection.

Pipeline: pCCA emission init, then :class:`VEMARDEngine` variational
EM (E-step with the exact dense ``T × T`` GP covariance, LBFGS on the
GP hyperparameters, closed-form ARD updates). The dense-GP path is
``O(T³)`` per Cholesky.

Outputs: standard layout plus ``ard_alpha.png`` showing per-column α
(large bars indicate pruned columns).
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="mDLAG (time-domain GP) on 3-region data.")
    parser.add_argument("--out-dir", type=Path, default=Path("examples/synthetic/demo_outputs/mdlag_time"))
    ...
    # Fit. VEMARDEngine uses LBFGS on the GP hyperparameters.
    ...
    print(f"[demo_mdlag_time] device={args.device}  out_dir={args.out_dir}")
    scenario = build_scenario(args)
    print(
        f"[demo_mdlag_time] scenario: T={scenario.T} B={scenario.n_trials} "
    ...
    print("[demo_mdlag_time] initializing from data (pCCA) ...")
    model.initialize_from_data(data)

    print(f"[demo_mdlag_time] fitting ({args.num_iters} iters) ...")
    t0 = time.perf_counter()
    result = model.fit(data, max_iter=args.num_iters, tol=1e-8)
    wall_s = time.perf_counter() - t0
    print(f"[demo_mdlag_time] fit done in {wall_s:.1f}s  ({result.n_iter} iters)")
    ...
    demo.print_summary(record, "mdlag_time")
