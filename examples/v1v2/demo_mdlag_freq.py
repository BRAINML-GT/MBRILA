"""mDLAG (frequency-domain VEM) on real V1/V2 visual-cortex recordings.

Real-data counterpart of :mod:`examples.synthetic.demo_mdlag_freq`.
The circulant frequency-domain approximation is roughly 22× faster
than the dense time-domain engine at the cost of a small δ bias
(~0.1 bin in synthetic recovery tests with T=100); the bias shrinks
with T as the circulant boundary effect becomes negligible.

Pipeline:

1. Load V1/V2 → train/val/test split.
2. pCCA emission init via ``model.initialize_from_data()``.
3. Fit via ``model.fit()`` → :class:`VEMARDFreqEngine` outer VEM.
...
from mbrila import MDLAG, ARDPriorConfig, LatentSpec, MOSEKernel, VEMARDFreqEngine


def main() -> None:
    parser = argparse.ArgumentParser(description="mDLAG (frequency-domain VEM) on real V1/V2 data.")
    parser.add_argument(
        "--data-path",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "data" / "demo_v1v2_data.pkl",
    )
    parser.add_argument("--out-dir", type=Path, default=Path("examples/v1v2/demo_outputs/mdlag_freq"))
    ...
    print(f"[demo_mdlag_freq] data: y={tuple(y_all.shape)}  y_dims={y_dims}")
    ...
    print(
        f"[demo_mdlag_freq] split: train={train_data.y.shape[0]}  "
    ...
    print(
        f"[demo_mdlag_freq] co-smoothing on TEST  "
    ...
    summary = v1v2.write_v1v2_outputs(
        ...
        model_label="mDLAG (frequency-domain VEM)",
    ...
    v1v2.print_summary(summary, "mdlag_freq", region_names)
