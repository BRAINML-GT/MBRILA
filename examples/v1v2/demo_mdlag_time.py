"""mDLAG (time-domain GP) on real V1/V2 visual-cortex recordings.

Real-data counterpart of :mod:`examples.synthetic.demo_mdlag_time`.
mDLAG uses ARD on the loading matrix C to soft-prune redundant latent
columns — the user passes a generous ``--n-across`` and reads off the
effective rank from the per-column α at the end.

Pipeline:

1. Load V1/V2 → train/val/test split.
2. pCCA emission init via ``model.initialize_from_data()``.
3. Fit via ``model.fit()`` → :class:`VEMARDEngine` outer VEM.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="mDLAG (time-domain GP) on real V1/V2 data.")
    parser.add_argument(
        "--data-path",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "data" / "demo_v1v2_data.pkl",
    )
    parser.add_argument("--out-dir", type=Path, default=Path("examples/v1v2/demo_outputs/mdlag_time"))
    ...
    print(f"[demo_mdlag_time] data: y={tuple(y_all.shape)}  y_dims={y_dims}")
    ...
    print(
        f"[demo_mdlag_time] split: train={train_data.y.shape[0]}  "
    ...
    print(
        f"[demo_mdlag_time] co-smoothing on TEST  "
    ...
    v1v2.print_summary(summary, "mdlag_time", region_names)
